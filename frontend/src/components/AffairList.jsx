...
const AffairList = ({ subject, dateRange, topic }) => {
  ...
  useEffect(() => {
    const fetchAffairs = async () => {
      try {
        ...
        const res = await axios.get(`${process.env.REACT_APP_BACKEND_URL}/api/affairs`, {
          params,
        });
        setAffairs(res.data);
      } catch (err) {
  ...
  const handleDelete = async (id) => {
    const token = localStorage.getItem("token");
    try {
      await axios.delete(`${process.env.REACT_APP_BACKEND_URL}/api/affairs/${id}`, {
    ...
    <div>
      ...
      {affairs.map((item) => (
        ...
        >
          ...
          {item.photo && (
            <img
              src={`${process.env.REACT_APP_BACKEND_URL}/uploads/${item.photo}`}
